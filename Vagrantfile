  ...
  # UCP 2.1 node for DDC
    config.vm.define "ucp-nfs-node1" do |ucp_nfs_node1|
      ...
      ucp_nfs_node1.vm.provision "shell", inline: <<-SHELL
       ...
       docker run --rm --name ucp -v /var/run/docker.sock:/var/run/docker.sock docker/ucp:2.1.0 install --host-address ${UCP_IPADDR} --admin-password ${UCP_PASSWORD} --san ucp.local --license $(cat docker_subscription.lic)
